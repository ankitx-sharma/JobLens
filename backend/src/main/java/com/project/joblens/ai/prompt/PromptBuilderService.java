package com.project.joblens.ai.prompt;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.project.joblens.entity.PromptsEntity;
import com.project.joblens.repository.PromptRepository;
...
public class PromptBuilderService {
	...
	public String buildStructuredJsonPrompt(String jobDescriptionText, PromptVersion version) {
		return switch (version) {
			case V1 -> buildVersion1(jobDescriptionText);
		};
	}
	
	private String buildVersion1(String jobDescriptionText) {
		PromptsEntity prompt = repository.findByPromptNameAndVersionAndActiveTrue("JOB_ANALYSIS", "V1").orElse(getDefaultVersion());
		return prompt.getPromptText().formatted(jobDescriptionText);
	}
	...
	private PromptsEntity getDefaultVersion() {
		...
		PromptsEntity promptDefault = new PromptsEntity();
		promptDefault.setPromptName("JOB_ANALYSIS");
		promptDefault.setPromptText(prompt);
		promptDefault.setVersion("V1");
